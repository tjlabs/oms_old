...
def calculate_time_to_first_fix(db_conn: postgresDBModule.DBConnection, one_day_whole_test_sets: list, start_time: datetime, end_time: datetime, sector_id: int) -> models.TimeToFirstFix:
    trial_count, total_time = 0, 0
    hour_unit_ttff, hour_unit_cnt = [0.0]*24, [0]*24
    ...
    for one_user in one_day_whole_test_sets:
        for one_test in one_user.test_sets:
            ...
            phase_four_time = get_phase_four_time(db_conn, one_test.start_time, end_time, one_user.user_id)
            ttff = calculate_ttff(one_test.start_time, phase_four_time)

            total_time += ttff
            trial_count += 1

            hour_unit_ttff[one_test.start_time.hour] += ttff
            hour_unit_cnt[one_test.start_time.hour] += 1
    ...
    stabilization_info = models.TimeToFirstFix(
        ...
        hour_unit_ttff=average_hour_unit_data(hour_unit_ttff, hour_unit_cnt),
